...
ax[0].set_yticklabels(np.arange(0,110,20),fontsize=16)
ax[0].set_xlim(0,20)
ax[0].set_xticks(np.arange(0,21,2.5))
ax[0].set_xticklabels([])
ax[0].set_ylabel('S/N',fontsize=18)
...
legend = ax[0].legend(loc='upper right',numpoints=1,fontsize=18)

#ax.text(2.8, 80, r'$\delta\nu=100$')

ax[1].set_ylim(2,8)
ax[1].set_yticks(np.arange(2,8,2))
ax[1].set_yticklabels(np.arange(2,8,2),fontsize=16)
ax[1].set_xlim(0,20)
ax[1].set_xticks(np.arange(0,21,2))
ax[1].set_xticklabels([0,'','','','',0.1,'','','','',0.2],fontsize=16)
ax[1].set_ylabel(r'$(S/N)_{\rm{I}}/(S/N)_{\rm{var}}$',fontsize=18)
ax[1].set_xlabel(r'Noise level $\sigma_{\rm{n}}$ (arbitary units)',fontsize=18)
